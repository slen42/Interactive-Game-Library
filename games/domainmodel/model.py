...
class Game:
    ...
    # added this temporarily to show reviews
    # todo in future assignments? for each review, create user or assign to existing user
    def add_review_dodgy(self, review: str):
...
def make_genre_association(game: Game, genre: Genre):
    if genre.is_applied_to(game):
        raise ModelException(
            f'Genre {genre.genre_name} already applied to Game "{game.title}"')
...
class User:
    def __init__(self, username: str, password: str):
        ...
        if isinstance(password, str) and len(password) >= 7:
            ...
            raise ValueError(
                f'Your password was only {len(password)} characters long. Ensure it is at least 7 characters long.')
...
class Review:
    ...
    def __str__(self):
        return f"Game: {self.game.title}, Rating: {self.rating}, Comment: {self.comment}"
